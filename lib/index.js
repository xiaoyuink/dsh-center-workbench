/**
 * dsh-workbench Host half —— 独立中栏面板的数据接口。
 *
 * 提供若干 HTTP 路由 + 一个 WebSocket 升级路由供浏览器端面板使用：
 *   - GET  /api/dsh-workbench/tree  ?path=<绝对路径> -> 列出目录条目
 *   - GET  /api/dsh-workbench/file  ?path=<绝对路径> -> 读取文本文件
 *   - POST /api/dsh-workbench/file/save {path,text}  -> 写回文件
 *   - POST /api/dsh-workbench/file/upload ?dir=&name= (原始二进制) -> 写入外部导入的文件
 *   - GET  /api/dsh-workbench/tree/recursive ?path=<绝对路径> -> 递归列出目录（相对路径）
 *   - GET  /api/dsh-workbench/env  -> 环境信息（用户主目录/平台/分隔符，供跨平台默认根目录）
 *   - GET  /api/dsh-workbench/tasks/live ?root=<会话id>  -> 运行中子代理的实时活动
 *   - POST /api/dsh-workbench/tasks/output {sessionId,id} -> 重放模型已读的任务输出
 *   - POST /api/dsh-workbench/tasks/kill {sessionId,id}  -> 终止后台任务
 *   - GET  /api/dsh-workbench/terminal （WebSocket 升级）-> 简化终端（子 shell 管道）
 *
...
/** 文件字节响应的 ETag（mtime + size）。 */
function fileEtag(st) {
  return `"${Math.floor(st.mtimeMs).toString(16)}-${st.size.toString(16)}"`
}

/**
 * 带缓存头的文件字节响应：ETag（mtime + size）+ `max-age=0, must-revalidate` + 条件请求。
 *
 * 预览标签来回切换会反复挂载/卸载，其中图片（`<img src>`）与 PDF（`<iframe src>`）由
 * 浏览器自己取 URL，插不进客户端的内存缓存；没有 ETag 时每次都是整份重新下载。
 * 用 `max-age=0, must-revalidate` 而不是给一段 max-age：每次显示都问一次服务器，
 * 文件没变就只回一个**不带 body 的 304**（字节不再重传、磁盘也不再重读），因此既
 * 不重复下载，也不会有"改了文件但预览还是旧的"这种陈旧窗口。
 */
function sendFileBytes(req, res, st, contentType) {
  const etag = fileEtag(st)
  const raw = req.headers && req.headers['if-none-match']
  if (typeof raw === 'string' && raw.split(',').map((s) => s.trim()).indexOf(etag) >= 0) {
    res.statusCode = 304
...
function createJobOutputMirror(ctx) {
  ...
  if (typeof ctx.effect === 'function') ctx.effect(() => dispose, 'dsh-workbench: job-output event mirror')
  return { entries: (sessionId) => perSession.get(sessionId) || [] }
}

export function apply(ctx) {
  // —— 诊断信标（临时）：客户端把「是否拿到官方侧边栏服务 / 是否已注册标签 / 报错」
  //    以 JSONL 追加写到 ~/.dsh/dsh-workbench/diag.jsonl，便于在没有浏览器
  //    控制台的情况下定位客户端装配问题。排查完可删（连同 client.js 的 beacon()）。
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/diag',
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req)
        const dir = join(homedir(), '.dsh', 'dsh-workbench')
        await mkdir(dir, { recursive: true })
        const line = JSON.stringify({ at: new Date().toISOString(), ...body }) + '\n'
        await writeFile(join(dir, 'diag.jsonl'), line, { flag: 'a' })
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/file/save',
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req)
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const st = await stat(path)
        if (st.isDirectory()) return sendJson(res, 400, { ok: false, error: 'is a directory' })
        if (sendFileBytes(req, res, st, mimeFromExt(path))) return
        const buf = await readFile(path)
        res.end(buf)
      } catch (error) {
        res.statusCode = 404
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/file/op',
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req)
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/file/del',
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req)
        const path = typeof body?.path === 'string' ? body.path : ''
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/file/create',
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req)
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/file/upload',
    handler: async (req, res) => {
      try {
        const url = new URL(req.url ?? '/', 'http://x')
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/tree/recursive',
    handler: async (req, res) => {
      try {
        const path = queryPath(req)
        if (!isAbsolute(path)) return sendJson(res, 400, { ok: false, error: 'path must be absolute' })
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/tasks/live',
    handler: async (req, res) => {
      try {
        const url = new URL(req.url ?? '/', 'http://x')
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/tasks/output',
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req)
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-workbench/tasks/kill',
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req)
        const sessionId = typeof body?.sessionId === 'string' ? body.sessionId : ''
  ...
  const wssDispose = ctx.webServer.registerUpgrade({
    path: '/api/dsh-workbench/terminal',
  ...
  const registerVendorStatic = (path, file, contentType) => {
    ctx.webServer.register({
      kind: 'exact',
      path,
      handler: async (req, res) => {
  ...
  const registerXtermStatic = (path, specifier, contentType) => registerVendorStatic(path, req.resolve(specifier), contentType)
  registerXtermStatic('/api/dsh-workbench/xterm.js', '@xterm/xterm/lib/xterm.js', 'application/javascript; charset=utf-8')
  registerXtermStatic('/api/dsh-workbench/addon-fit.js', '@xterm/addon-fit/lib/addon-fit.js', 'application/javascript; charset=utf-8')
  registerXtermStatic('/api/dsh-workbench/xterm.css', '@xterm/xterm/css/xterm.css', 'text/css; charset=utf-8')

  // —— Office 预览库（docx/xlsx/pptx/csv）：浏览器端 <script> 懒加载 ——
  // docx-preview 的 UMD 依赖全局 JSZip（须先加载）；xlsx 为 SheetJS UMD；
  // pptx-preview 已用 esbuild 预打包为 IIFE 静态资产（lib/vendor/pptx-preview.bundle.js）。
  const jszipFile = join(dirname(req.resolve('jszip/package.json')), 'dist/jszip.min.js')
  const xlsxFile = join(dirname(req.resolve('xlsx/package.json')), 'dist/xlsx.full.min.js')
  const docxFile = join(dirname(req.resolve('docx-preview')), 'docx-preview.min.js')
  registerVendorStatic('/api/dsh-workbench/vendor/jszip.js', jszipFile, 'application/javascript; charset=utf-8')
  registerVendorStatic('/api/dsh-workbench/vendor/docx-preview.js', docxFile, 'application/javascript; charset=utf-8')
  registerVendorStatic('/api/dsh-workbench/vendor/xlsx.js', xlsxFile, 'application/javascript; charset=utf-8')
  registerVendorStatic('/api/dsh-workbench/vendor/pptx-preview.js', fileURLToPath(new URL('./vendor/pptx-preview.bundle.js', import.meta.url)), 'application/javascript; charset=utf-8')

  // —— 图片扩展预览库（TIFF/HEIC/PSD，浏览器端 <script> 懒加载）——
  // UTIF.js 依赖全局 pako（须先加载）；heic2any 内置 wasm（离线可用）；ag-psd 为自带 UMD bundle。
  const pakoFile = join(dirname(req.resolve('pako/package.json')), 'dist/pako.min.js')
  const agpsdFile = join(dirname(req.resolve('ag-psd/package.json')), 'dist/bundle.js')
  registerVendorStatic('/api/dsh-workbench/vendor/pako.js', pakoFile, 'application/javascript; charset=utf-8')
  registerVendorStatic('/api/dsh-workbench/vendor/utif.js', req.resolve('utif'), 'application/javascript; charset=utf-8')
  registerVendorStatic('/api/dsh-workbench/vendor/heic2any.js', req.resolve('heic2any'), 'application/javascript; charset=utf-8')
  registerVendorStatic('/api/dsh-workbench/vendor/ag-psd.js', agpsdFile, 'application/javascript; charset=utf-8')
}
