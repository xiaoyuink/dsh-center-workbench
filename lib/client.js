...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS_TEXT = [
      ...
      ".dsh-cs-task-jobs-settled{opacity:.55}",
      ".dsh-cs-task-jobs-kill-error{flex:none;color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-11)}",
    ...
    function TaskOutputDock(props) {
      ...
      return createElement("div", { className: "dsh-cs-task-jobs-pane" }, [
        createElement("div", { className: "dsh-cs-task-jobs-pane-header" }, [
          createElement("span", { className: "dsh-cs-task-jobs-dot " + (job.status === "running" ? "dsh-cs-task-dot-running" : job.status === "failed" ? "dsh-cs-task-dot-error" : job.status === "stopping" || job.status === "killed" ? "dsh-cs-task-dot-warn" : "dsh-cs-task-dot-done") }),
      ...
      return createElement("div", { className: "dsh-cs-task-jobs" }, [
        ...
        createElement("ul", { className: "dsh-cs-task-jobs-list" }, rows.map(function (row) {
          ...
          return createElement("li", { key: job.id, className: "dsh-cs-task-jobs-row" + (!live ? " dsh-cs-task-jobs-settled" : "") + (selected ? " dsh-cs-task-jobs-selected" : "") }, [
            ...
            job.status === "running" ? createElement("button", { type: "button", className: "dsh-cs-task-jobs-kill" + (armed ? " dsh-cs-task-jobs-kill-armed" : ""), title: armed ? "再点一次确认终止" : "终止任务", disabled: killing, onClick: function (e) { e.stopPropagation(); if (armed) kill(row); else setArmedId(job.id); } }, armed ? "确认终止" : "终止") : null,
    ...
    function TasksView(props) {
      ...
      // 页面可见期间观察根 + 所有分支；根切换/隐藏/卸载时全部释放。
      useEffect(function () {
        if (!active || rootId === undefined || !hasCatalogs) return;
        observe(rootId);
        branches.forEach(observe);
        return function () { unobserveAll(); };
      }, [branches, rootId, active, hasCatalogs]);
      ...
      function renderCatalog(parentId, level) {
        ...
        return entries.map(function (entry) {
          ...
          return createElement("div", { key: entry.id, className: "dsh-cs-task-node", style: { paddingLeft: (level * 14) + "px" } }, [
            createElement("button", { type: "button", role: "treeitem", tabIndex: 0, "aria-level": level, "aria-expanded": entry.hasChildren ? "true" : undefined, "aria-current": isCurrent ? "true" : undefined, "aria-label": label + " " + secondary, className: "dsh-cs-task-row" + (isCurrent ? " dsh-cs-task-row-active" : ""), onClick: function () { openChild(entry.id, parentId); } }, [
      ...
      if (rootId === undefined) {
        ...
        topologyBody = createElement("div", { className: "dsh-cs-tasks-tree", role: "tree", "aria-label": "子代理拓扑" }, [
          ...
          readyEmpty ? createElement("div", { className: "dsh-cs-tasks-empty" }, [
            ...
            createElement("div", { className: "dsh-cs-tasks-empty-hint" }, "当前主代理尚未创建子代理；运行子代理时会自动出现。"),
          ]) : null,
        ]);
      }
      ...
      /** 子代理模式的中文标签。 */
      function modeLabelOf(mode) { return mode === "one-shot" ? "一次性" : "可持续"; }
      /** 目录诊断原因的中文标签。 */
