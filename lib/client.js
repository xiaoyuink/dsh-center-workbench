    ...
    *  立即判定会把线程首帧误认为新子代理；延迟后用实时快照按原基线重评估。 */
    ...
    function SidebarPreviewBody(props) {
      ...
      return createElement("div", { className: "dsh-cs-sb-body dsh-cs-preview", "data-dsh-cs-sidebar-body": "preview" },
        createElement("div", { className: "dsh-cs-editor-bar" }, [
          ...
          createElement("button", { type: "button", className: "dsh-cs-editor-save", "data-dirty": snap.dirty ? "1" : undefined, title: "保存到磁盘", disabled: !snap.editable, onClick: save }, "保存"),
          createElement("button", { type: "button", className: "dsh-cs-editor-close", title: "关闭这个预览标签", onClick: close }, "关闭"),
        ]),
    ...
    function WorkbenchHeaderButton(props) {
      ...
      useEffect(function () {
        function sync() {
          var on = false;
          try { on = sidebarRight.isExpanded() && (sidebarRight.active() || {}).kind === SB_EXPLORER_KIND; } catch (e) { on = false; }
          setActive(on);
        }
        sync();
      ...
      var label = active ? "聚焦工作台" : "打开工作台";
      return createElement("button", {
        type: "button",
        "data-dsh-cw-header-action": "true",
        "aria-pressed": active ? "true" : "false",
        "aria-label": "工作台",
        title: label + "（资源管理器 / 浏览器 / 终端 / 后台任务）",
        onClick: function () {
          try { sidebarRight.openTab(SB_EXPLORER_KIND); } catch (e) { /* 无会话停靠面：无处可开 */ }
        },
        style: {
